    ...
    if($validate == 'validado'){
        ...
        switch($methodApi){
            ...
            case 'POST':
                ...
                $metodo = $_POST['metodo'];
                //verificamos con la variable metodo que tipo de orden es
                //si garantia o reintegro
                if($metodo == 'garantia'){
                    //si es garantia insertamos en almacen garantia y actualizamos el 
                    ...
                    if($resInsert and $resUpdate){
                        //ejecutar commit si las operaciones son efectivas
                        $con->commit();
                    ...
                    /* si no es el metodo garantia , es decir es reintegro
                    verificamos si el producto existe en el almacen del piso 1
                    */
                    $con->autocommit(false);
                    $sqlSelect = 'SELECT * FROM productos_almacenes WHERE id='.$_POST['id_producto'].' AND id_almacen=15';
                    ...
                    if($fill){
                        ...
                        $sqlUpdateRegistro = 'UPDATE registro_usuario SET cantidad=cantidad-'.$_POST['cantidad'].' WHERE id_producto='.$_POST['id_producto'].' AND orden="'.$_POST['orden'].'"';
                        $resUpdateRegistro = mysqli_query($con,$sqlUpdateRegistro);

                        if($resUpdate and $resUpdateProductos and $resUpdateRegistro){
                            $con->commit();
                            header("HTTP/1.1 200 OK");
                            $response['mensaje'] = 'Producto transferido exitosamente';
                            $response['status'] = 200;
                        ...
                        if($resInsert and $resUpdateProductos and $resUpdateRegistro){
                            $con->commit();
                            header("HTTP/1.1 200 OK");
                            $response['mensaje'] = 'Producto transferido exitosamente';
                            $response['status'] = 200;
